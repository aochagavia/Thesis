...
We can see that both normal forms have the same semantics but are still syntactically different. In this case, it seems reasonable to think of a semantics-preserving tranformation that transforms the right-hand side of Cluster B into \haskell{map words}. Figuring out which exact transformation we need is the role of the \emph{enhance} step, explained in the next section.

% FIXME: talk about the higher-level problem of students writing xs, instead of currying
...
% FIXME: mention let inlining, since we are using it bellow

One possible solution is to add a normalization step that, for any \haskell{case} expression consisting of a single alternative, rewrites it in terms of \haskell{let}. In the case of Cluster B, this transformation will allow the inliner to simplify the code further in a later pass, which in turn will open the doors to $\eta$-reduction.
...
Such simplicity hints at the possibility of using an automated theorem prover to unify full programs. For a more detailed discussion see Chapter \ref{sec:discussion}.
